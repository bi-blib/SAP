...
def format_hex_to_single_byte_string(hex_string):
    # Convert the hex string to a single bytes object
    byte_data = bytes.fromhex(hex_string)
    
    # Format each byte as \x00 and join them into a single string
    formatted_byte_string = "b'" + ''.join(f'\\x{byte:02X}' for byte in byte_data) + "'"
    
    return formatted_byte_string

def save_to_file(filename, data_list):
    with open(filename, 'w') as file:
        # Write each formatted key on a new line
        for data in data_list:
            file.write(data + ",\n")

# Collect all formatted keys
formatted_keys = [format_hex_to_single_byte_string(hex_string) for hex_string in knownKeys]

# Save all formatted keys to a new file
output_filename = "formattedKeys.py"
save_to_file(output_filename, formatted_keys)
